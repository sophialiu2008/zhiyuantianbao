create table if not exists public.admission_plan_history_matches (
  ...
  min_score_2023 int,
  min_rank_2023 int,
  history_years int not null default 0,
  best_rank int,
  worst_rank int,
  avg_rank int,
  latest_score int,
  latest_rank int,
  is_new_major boolean not null default false,
...
create index if not exists idx_admission_plan_history_matches_lookup
  on public.admission_plan_history_matches (
    plan_year, subject, batch, school_code, major_code, major_name_normalized
  );

create index if not exists idx_admission_plan_history_matches_norm
  on public.admission_plan_history_matches (
    plan_year, subject, batch, school_name_normalized, major_name_normalized
  );

alter table public.admission_plan_history_matches enable row level security;

drop policy if exists "public read admission plan history matches" on public.admission_plan_history_matches;
create policy "public read admission plan history matches"
on public.admission_plan_history_matches for select
...
as $$
  ...
  plan_rows as (
    select
      ...
      loc.city,
      loc.campus_city,
      coalesce(pm.history_years, h.history_years, 0) as history_years,
      coalesce(pm.best_rank, h.best_rank) as best_rank,
      coalesce(pm.worst_rank, h.worst_rank) as worst_rank,
      coalesce(pm.avg_rank, h.avg_rank) as avg_rank,
      coalesce(pm.latest_score, h.latest_score) as min_score,
      coalesce(pm.latest_rank, h.latest_rank) as min_rank,
      coalesce(
        (
          select jsonb_agg(
            jsonb_build_object(
      ...
      case
        when coalesce(pm.worst_rank, h.worst_rank) is null then null
        else coalesce(pm.worst_rank, h.worst_rank) - p_user_rank
      end as rank_diff,
      case
        when pm.id is not null then 'precomputed_xlsx'
        when coalesce(h.history_years, 0) > 0 then 'normalized_name'
        else 'none'
