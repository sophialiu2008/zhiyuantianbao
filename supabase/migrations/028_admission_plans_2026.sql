create table if not exists public.admission_plans (
  id bigserial primary key,
  year int not null,
  subject text not null check (subject in ('physics', 'history')),
  batch text not null,
  plan_nature text not null default '非定向',
  volunteer_mode text not null default '',
  school_code text not null,
  school_name text not null,
  school_name_raw text not null,
  school_name_normalized text not null,
  school_tags text[] not null default '{}'::text[],
  major_code text not null,
  major_name text not null,
  major_name_normalized text not null,
  major_remark text not null default '',
  subject_requirement text not null default '',
  plan_count int,
  duration_years int,
  tuition int,
  source_file text not null,
  source_page int,
  source_row int,
  entry text,
  crawled_at timestamptz,
  raw_payload jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (
    year, subject, batch, plan_nature, volunteer_mode,
    school_code, major_code, major_name, major_remark
  )
);

create index if not exists idx_admission_plans_lookup
  on public.admission_plans (year, subject, batch, school_code, major_code);

create index if not exists idx_admission_plans_school_trgm
  on public.admission_plans using gin (school_name_normalized gin_trgm_ops);

create index if not exists idx_admission_plans_major_trgm
  on public.admission_plans using gin (major_name gin_trgm_ops);

create index if not exists idx_admission_plans_tags
  on public.admission_plans using gin (school_tags);

alter table public.admission_plans enable row level security;

drop policy if exists "public read admission plans" on public.admission_plans;
create policy "public read admission plans"
on public.admission_plans for select
to anon, authenticated
using (true);

insert into public.data_sources (source_key, source_name, source_type, source_year, credibility, update_frequency, notes)
values (
  'hebei_admission_plans_2026',
  '河北省2026年普通高校招生计划',
  '官方招生计划',
  2026,
  '高',
  '每年6月',
  '用于以当年招生计划为主表，关联历史投档位次、院校画像、专业画像和就业数据。'
)
on conflict (source_key) do update set
  source_name = excluded.source_name,
  source_type = excluded.source_type,
  source_year = excluded.source_year,
  credibility = excluded.credibility,
  update_frequency = excluded.update_frequency,
  notes = excluded.notes,
  updated_at = now();

create or replace function public.recommend_admission_plans(
  p_plan_year int default 2026,
  p_history_year int default 2025,
  p_subject text default 'physics',
  p_user_rank int default 0,
  p_query text default '',
  p_risk text default 'all',
  p_tag text default 'all',
  p_limit int default 50,
  p_offset int default 0,
  p_provinces text[] default '{}',
  p_cities text[] default '{}',
  p_batches text[] default '{}',
  p_subject_requirements text[] default '{}'
)
returns table (
  id bigint,
  year int,
  subject text,
  school_code text,
  school_name text,
  school_tags text[],
  province text,
  city text,
  campus_city text,
  major_code text,
  major_name text,
  min_score int,
  min_rank int,
  admission_type text,
  remark text,
  history_years int,
  best_rank int,
  worst_rank int,
  avg_rank int,
  basis_rank int,
  rank_diff int,
  risk_type text,
  history jsonb,
  total_count bigint,
  plan_id bigint,
  plan_year int,
  batch text,
  plan_nature text,
  volunteer_mode text,
  plan_count int,
  duration_years int,
  tuition int,
  subject_requirement text,
  major_remark text,
  match_confidence text
)
language sql
stable
security definer
set search_path = public
as $$
  with query_terms as (
    select array_remove(regexp_split_to_array(coalesce(p_query, ''), '\s*[,，、]\s*'), '') as terms
  ),
  plan_rows as (
    select
      p.*,
      coalesce(sp.school_tags, p.school_tags) as profile_school_tags,
      coalesce(sp.is_985, false) as profile_is_985,
      coalesce(sp.is_211, false) as profile_is_211,
      coalesce(sp.is_double_first_class, false) as profile_is_double_first_class,
      sp.ownership as profile_ownership,
      loc.province,
      loc.city,
      loc.campus_city
    from public.admission_plans p
    left join public.school_profiles sp
      on sp.school_name_normalized = p.school_name_normalized
      or sp.school_code = p.school_code
    left join public.school_locations loc
      on loc.school_name_normalized = p.school_name_normalized
    cross join query_terms qt
    where p.year = p_plan_year
      and p.subject = p_subject
      and (
        cardinality(qt.terms) = 0
        or exists (
          select 1
          from unnest(qt.terms) term
          where trim(term) <> ''
            and (
              p.school_name ilike '%' || trim(term) || '%'
              or p.major_name ilike '%' || trim(term) || '%'
              or p.school_code ilike '%' || trim(term) || '%'
              or p.major_code ilike '%' || trim(term) || '%'
              or p.major_remark ilike '%' || trim(term) || '%'
            )
        )
      )
      and (
        p_tag = 'all'
        or (
          p_tag = '普通本科'
          and p.batch like '本科%'
          and not coalesce(sp.is_985, false)
          and not coalesce(sp.is_211, false)
          and not coalesce(sp.is_double_first_class, false)
          and coalesce(sp.ownership, '') not in ('民办', '中外合作办学', '内地与港澳台合作办学')
          and not ('民办' = any(coalesce(sp.school_tags, p.school_tags)))
          and not ('民办本科' = any(coalesce(sp.school_tags, p.school_tags)))
          and not ('独立学院' = any(coalesce(sp.school_tags, p.school_tags)))
          and not ('中外合作办学' = any(coalesce(sp.school_tags, p.school_tags)))
          and not ('内地与港澳台地区合作办学' = any(coalesce(sp.school_tags, p.school_tags)))
          and not ('内地与港澳台合作办学' = any(coalesce(sp.school_tags, p.school_tags)))
          and not ('香港' = any(coalesce(sp.school_tags, p.school_tags)))
        )
        or (
          p_tag <> '普通本科'
          and (
            p_tag = any(coalesce(sp.school_tags, p.school_tags))
            or (p_tag = '985' and coalesce(sp.is_985, false))
            or (p_tag = '211' and coalesce(sp.is_211, false))
            or (p_tag = '双一流' and coalesce(sp.is_double_first_class, false))
            or p.major_name ilike '%' || p_tag || '%'
            or p.major_remark ilike '%' || p_tag || '%'
          )
        )
      )
      and (
        cardinality(coalesce(p_provinces, '{}')) = 0
        or loc.province = any(p_provinces)
      )
      and (
        cardinality(coalesce(p_cities, '{}')) = 0
        or loc.city = any(p_cities)
        or coalesce(loc.campus_city, loc.city) = any(p_cities)
      )
      and (
        cardinality(coalesce(p_batches, '{}')) = 0
        or p.batch = any(p_batches)
      )
      and (
        cardinality(coalesce(p_subject_requirements, '{}')) = 0
        or p.subject_requirement = any(p_subject_requirements)
      )
  ),
  scored as (
    select
      p.*,
      h.history_years,
      h.best_rank,
      h.worst_rank,
      h.avg_rank,
      h.history,
      h.latest_score as min_score,
      h.latest_rank as min_rank,
      h.worst_rank as basis_rank,
      case when h.worst_rank is null then null else h.worst_rank - p_user_rank end as rank_diff,
      h.match_confidence,
      case
        when h.history_years = 0 then 'unknown'
        when p_user_rank <= 0 then 'unknown'
        when p_user_rank <= 20000 and (h.worst_rank - p_user_rank) < -2000 then 'unknown'
        when p_user_rank <= 20000 and (h.worst_rank - p_user_rank) < 0 then 'reach'
        when p_user_rank <= 20000 and (h.worst_rank - p_user_rank) <= 3000 then 'match'
        when p_user_rank <= 20000 then 'safe'
        when p_user_rank <= 80000 and (h.worst_rank - p_user_rank) < -3000 then 'unknown'
        when p_user_rank <= 80000 and (h.worst_rank - p_user_rank) < 0 then 'reach'
        when p_user_rank <= 80000 and (h.worst_rank - p_user_rank) <= 5000 then 'match'
        when p_user_rank <= 80000 then 'safe'
        when (h.worst_rank - p_user_rank) < -5000 then 'unknown'
        when (h.worst_rank - p_user_rank) < 0 then 'reach'
        when (h.worst_rank - p_user_rank) <= 8000 then 'match'
        else 'safe'
      end as risk_type
    from plan_rows p
    cross join lateral (
      select
        count(distinct hh.year)::int as history_years,
        min(hh.min_rank)::int as best_rank,
        max(hh.min_rank)::int as worst_rank,
        round(avg(hh.min_rank))::int as avg_rank,
        (array_agg(hh.min_score order by hh.year desc, hh.min_rank asc))[1] as latest_score,
        (array_agg(hh.min_rank order by hh.year desc, hh.min_rank asc))[1] as latest_rank,
        coalesce(
          jsonb_agg(
            jsonb_build_object(
              'year', hh.year,
              'min_score', hh.min_score,
              'min_rank', hh.min_rank,
              'major_code', hh.major_code
            )
            order by hh.year desc
          ) filter (where hh.id is not null),
          '[]'::jsonb
        ) as history,
        case
          when bool_or(hh.school_code = p.school_code and hh.major_code = p.major_code and hh.major_name = p.major_name) then 'code_and_name'
          when bool_or(public.normalize_school_name(hh.school_name) = p.school_name_normalized and public.normalize_major_name(hh.major_name) = p.major_name_normalized) then 'normalized_name'
          when bool_or(public.normalize_school_name(hh.school_name) = p.school_name_normalized and hh.major_name ilike p.major_name || '%') then 'school_major_prefix'
          when count(hh.id) > 0 then 'school_major_fuzzy'
          else 'none'
        end as match_confidence
      from public.admission_records hh
      where hh.subject = p.subject
        and hh.year between p_history_year - 2 and p_history_year
        and hh.min_rank is not null
        and public.normalize_school_name(hh.school_name) = p.school_name_normalized
        and (
          public.normalize_major_name(hh.major_name) = p.major_name_normalized
          or hh.major_name ilike p.major_name || '%'
          or p.major_name ilike hh.major_name || '%'
        )
    ) h
  ),
  filtered as (
    select *
    from scored
    where p_risk = 'all' or risk_type = p_risk
  )
  select
    f.id,
    f.year,
    f.subject,
    f.school_code,
    f.school_name,
    f.profile_school_tags as school_tags,
    f.province,
    f.city,
    f.campus_city,
    f.major_code,
    f.major_name,
    f.min_score,
    f.min_rank,
    f.plan_nature as admission_type,
    f.major_remark as remark,
    f.history_years,
    f.best_rank,
    f.worst_rank,
    f.avg_rank,
    f.basis_rank,
    f.rank_diff,
    f.risk_type,
    f.history,
    count(*) over() as total_count,
    f.id as plan_id,
    f.year as plan_year,
    f.batch,
    f.plan_nature,
    f.volunteer_mode,
    f.plan_count,
    f.duration_years,
    f.tuition,
    f.subject_requirement,
    f.major_remark,
    f.match_confidence
  from filtered f
  order by
    case f.risk_type when 'match' then 1 when 'safe' then 2 when 'reach' then 3 else 4 end,
    abs(coalesce(f.rank_diff, 999999999)),
    f.batch,
    f.school_name,
    f.major_code
  limit greatest(1, least(p_limit, 100))
  offset greatest(0, p_offset);
$$;
