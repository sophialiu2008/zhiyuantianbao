...
as $$
  ...
  left join lateral (
    select sp.*
    from public.school_profiles sp
    where sp.school_code = f.school_code
  ...
  left join lateral (
    select sp.*
    from public.school_profiles sp
    where sp_code.school_code is null
      and sp.school_name_normalized = f.school_name_normalized
    order by sp.school_name
    limit 1
  ) sp_name on true
  left join lateral (
    select loc.*
    from public.school_locations loc
    where loc.school_name_normalized = f.school_name_normalized
    order by loc.id
    limit 1
  ) loc on true
  order by
