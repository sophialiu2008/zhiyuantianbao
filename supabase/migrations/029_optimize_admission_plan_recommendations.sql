create index if not exists idx_admission_records_norm_history
...
create index if not exists idx_admission_plans_filter_2026
  on public.admission_plans (year, subject, batch, school_name_normalized, major_name_normalized);
...
as $$
  ...
  plan_rows as (
    ...
    left join lateral (
      ...
      where sp.school_name_normalized = p.school_name_normalized
        or sp.school_code = p.school_code
      order by
        case when sp.school_name_normalized = p.school_name_normalized then 0 else 1 end,
        sp.school_code
      limit 1
    ) sp on true
